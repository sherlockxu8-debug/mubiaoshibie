  ...
  <div class="page-camera">
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-button type="primary" size="small" @click="showAddDialog">添加摄像头</el-button>
      </div>
    </div>

    <div class="card">
      <el-table :data="cameras" v-loading="loading" stripe size="small">
        <el-table-column prop="id" label="编号" width="100" />
        <el-table-column prop="location" label="安装地点" min-width="160" />
...
const form = reactive({ id: '', location: '', building: '', floor: null, description: '' })
const rules = {
...
const loadCameras = async () => {
  loading.value = true
  try { cameras.value = await getCameras() }
...
const handleEdit = (camera) => {
  isEdit.value = true
  Object.assign(form, { id: camera.id, location: camera.location, building: camera.building || '', floor: camera.floor, description: camera.description || '' })
...
const handleSubmit = async () => {
  ...
  if (!valid) return
  try {
    isEdit.value ? await updateCamera(form.id, form) : await addCamera(form)
...
const handleDelete = async (camera) => {
  try {
    await ElMessageBox.confirm(`确定删除摄像头 ${camera.id} 吗？`, '删除确认', { type: 'warning' })
...
const resetForm = () => { Object.assign(form, { id: '', location: '', building: '', floor: null, description: '' }); formRef.value?.resetFields() }
